# /pact.contract — 行为契约生成

依据当前功能的 PID Card，生成行为契约文件。

> 文件读取见 CLAUDE.md § 4。契约生成的依据是 `specs/[功能名]-pid.md`（实例），不是 `templates/pid-card.md`（空白模板）。

---

## 入口检查

进入前先运行：

```bash
bash .pact/bin/pact-guard.sh contract
```

guard 失败则停止，不继续生成 contract。

### 检查 1 — PID Card 存在性

.pact/specs/[功能名]-pid.md 必须存在。
不存在则停止，提示先执行 /pact.pid。

### 检查 2 — PAD 占位符警告

若 specs/PAD.md 存在，读取并检测以下占位符标记：
  `[请补充`  `[实体名]`  `[角色]`  `[产品边界]`  `[目标]`  `[动作]`

发现任意一个时，输出：
```
⚠️ PAD.md 存在未填写字段：[列出具体字段名]
这些字段可能影响 FC 边界条件的准确性。
[A] 继续（接受 PAD 当前状态）
[B] 暂停，先完善 PAD.md
```
等待开发者选择，不自动继续。

### 检查 3 — PID 主流程映射完整性

PID Card 必须包含：
- PAD 业务主流程 Step
- 功能类型
- 成功后用户去向
- 架构影响
- 设计附件判断

缺失时输出：

```text
⚠️ PID Card 缺少主流程映射或架构影响。
[A] 返回 /pact.pid 补齐
[B] 继续，但在 contract 中标记“流程映射不完整”风险
```

等待人工选择，不自动继续。

### 检查 4 — 设计附件读取

若 PID 的“设计附件判断”声明需要 design brief / sequence / interaction：
- 对应附件文件必须存在。
- 读取附件中的“验收映射”。
- 将可自动化的条目转为 FC / NF 候选。
- 将无法自动化的条目标为人工验收。

若附件缺失，返回 /pact.pid 补齐，或在 contract 中明确记录“设计附件缺失”风险后由人工确认继续。

---

## 契约生成

参照 .pact/templates/contract.md 结构，根据 PID Card 内容逐条生成：
- **FC（功能契约）条目**：核心路径 + 边界条件
- **NF（非功能）条目**：性能、错误提示、交互反馈等
- **流程约束**：当前功能服务的 PAD 业务主流程 Step、成功后用户去向、必要状态变化
- **体验一致性约束**：继承 PAD 中与当前功能相关的空状态、错误提示、权限不足、重复操作等规则
- **设计附件约束**：若存在 design / sequence / interaction 附件，吸收其中的验收映射
- **明确不做**：在 PID Card 中标注的范围外功能

### 粒度检查

生成后执行一次契约粒度自查：
- FC 条目超过 7 条时，默认判定为疑似过大。
- 同一 contract 同时覆盖 Schema / 后端逻辑 / UI / 权限 / 异步任务中的 3 类以上时，默认判定为疑似过大。
- 若任一核心 FC 无法用真实命令、测试或人工验收方式独立验证，必须补充验证方式或拆分功能。

疑似过大时，输出：
```
⚠️ 契约粒度过大：[原因]
[A] 拆分为多个功能，回到 /pact.pid
[B] 生成 / 更新 exec-plan 后继续
```
等待人工选择，不自动推进。

输出路径：`.pact/contracts/[功能名].md`（严格遵守文件命名规范）

完成后更新 state.md：阶段字段写入 `contract`
