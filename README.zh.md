...
PACT 保持日常功能闭环轻量，但用两个全局主干约束功能：`PAD.md` 中的 Product Spine（产品主干）和 `architecture.md` 中的 Architecture Spine（架构主干）。功能进入 build 前，应说明自己位于哪条核心业务流程、触碰哪些模块和实体边界。
